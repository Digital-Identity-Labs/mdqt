Feature: Create sha1 alias to a metadata file
  ...
  Scenario Outline: A file is downloaded and then a link is created
    ...
    When I run `mdqt ln <file>`
    Then a symlink named <link_name> should be created
    And the exit status should be 0

    Examples:
      | entity | file | link_name |
  ...
  Scenario: The file to be linked to is an aggregate
    Given that I have defined an MDQ service
    And I have downloaded the metadata for aggregate to aggregate.xml
    When I run `mdqt ln aggregate.xml`
    Then the exit status should be 1
    And the output should contain "Error: "
